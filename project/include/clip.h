...
struct TextEncoder : GGMLNetwork {
    ...
    void pad_tokens(std::vector<int>& tokens, std::vector<float>& weights, bool padding = false)
    {
        ...
        // CheckPoint("text_model.n_token = %ld", text_model.n_token); // text_model.n_token = 77
    ...
    for (int i = 0; i < chunk_count; i++) {
        ...
        if (input_large_14) {
            for (int j = 0; j < CHUNK_LEN77; j++) {
                input_large_14->data[j] = (float)tokens->data[j];
            }
        } else {
        ...
        if (input_bigg_14) {
            for (int j = 0; j < CHUNK_LEN77; j++) {
                input_bigg_14->data[j] = (float)tokens->data[j];
            }
        } else {
        ...
        TENSOR *cond_latent[i] = clip->engine_forward(ARRAY_SIZE(argv), argv); // f32 [  2048,    77,     1,     1]
        ...
        if (cond_latent[i]) {
            for (int b = 0; b < cond_latent[i]->batch; b++) {
                for (int c = 0; c < cond_latent[i]->chan; c++) {
                    // f32 [  2048,    77,     1,     1]
                    // b * 77 + c
                    cond_latent[i]->data[b * cond_latent[i]->chan + c] *= chunk_weights->data[c];
                }
            }
        } // cond_latent[i]
    }
